...
$('.select').change(function () {
  const cartKey = $(this).attr('key')
  const checkStatus = $(this).prop('checked')

  if (checkStatus === true) {
    orderItems.push(cartKey)
  } else {
    const index = orderItems.indexOf(cartKey)
    ...
    success: data => {
      switch (data.result) {
        case 'pass':
          ...
          break
        default:
          alert('통신 중 오류가 발생하였습니다. 잠시 후 다시 시도해주세요.\n오류가 반복될 경우, 고객센터로 문의 부탁드립니다.\n(error_code: deleteCart.default)')
      }
    },
    error: err => {
      // 데이터베이스 오류 메시지 표시
      alert(`통신 중 오류가 발생하였습니다. 잠시 후 다시 시도해주세요.\n오류가 반복될 경우, 고객센터로 문의 부탁드립니다.\n(error_code: deleteCart.db.${err})`)
    }
  })
}
