		...
		<div id="outbox">
			...
			<div id="contents">
							...
							<c:forEach var="order" items="${orderList}">
								<tr>
									<td>
										...
										<p class="orderNum">
											<a href="/admin/order/user/0/detail/${order.orderNumber}">${order.orderNumber}</a>
										</p>
									</td>
									<td class="left">
										<a href="/admin/order/user/0/detail/${order.orderNumber}">${order.productName}
				...
				<div class="btnAreaList">
					...
					<div class="page-nav">
						<c:if test="${page != 1}">
							<a href="/admin/order/user/search/${type}/${keyword}/1" class="page-nav-nobox">
								<i class="fas fa-angle-double-left paging"></i>
							</a>
							<a href="/admin/order/user/search/${type}/${keyword}/${page - 1}" class="page-nav-nobox">
						...
						<c:forEach var="i" begin="${startPage}" end="${endPage}">
							<c:choose>
								<c:when test="${i == page}">
									<a href="/admin/order/user/search/${type}/${keyword}/${i}" class="page-nav-now">${i}</a>
								</c:when>
								<c:otherwise>
									<a href="/admin/order/user/search/${type}/${keyword}/${i}" class="page-nav-box">${i}</a>
								</c:otherwise>
							</c:choose>
						</c:forEach>
						...
						<c:if test="${page != pages && !orderList.isEmpty()}">
							...
							<a href="/admin/order/user/search/${type}/${keyword}/${pages}" class="page-nav-nobox">
