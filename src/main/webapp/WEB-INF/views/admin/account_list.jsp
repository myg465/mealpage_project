		...
		<div id="outbox">
			...
			<div id="contents" style="position: relative; top: -35px;">
						...
						<tbody>
							<c:forEach var="account" items="${accountList}">
								<tr>
									...
									<td>
										<ul class="state">
											<li class="r5"><a href="/admin/account/list/${page}/modify/${account.seqKey}" class="obtnMini iw40">수정</a></li>
											<li><a href="/admin/account/list/delete/${account.seqKey}/${account.id}" class="nbtnMini iw40" onclick="return confirm('정말로 해당 회원내역을 삭제(강제탈퇴)하시겠습니까?')">삭제</a></li>
											<li><a href="/admin/account/list/${page}/read/${account.seqKey}" class="decidebtn">상세보기</a></li>
										</ul>
									</td>
								</tr>
							</c:forEach>
				...
				<div class="btnAreaList">
					...
					<div class="page-nav">
						<c:if test="${page != 1}">
							<a href="/admin/account/list/1" class="page-nav-nobox">
								<i class="fas fa-angle-double-left paging"></i>
							</a>
							<a href="/admin/account/list/${page - 1}" class="page-nav-nobox">
						...
						<c:forEach var="i" begin="${startPage}" end="${endPage}">
							<c:choose>
								<c:when test="${i == page}">
									<a href="/admin/account/list/${i}" class="page-nav-now">${i}</a>
								</c:when>
								<c:otherwise>
									<a href="/admin/account/list/${i}" class="page-nav-box">${i}</a>
								</c:otherwise>
							</c:choose>
						</c:forEach>
						...
						<c:if test="${page != pages && !orderList.isEmpty()}">
							...
							<a href="/admin/account/list/${pages}" class="page-nav-nobox">
